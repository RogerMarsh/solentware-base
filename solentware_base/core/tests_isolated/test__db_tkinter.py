...
class Database_open_database(_DB):
    ...
    def test_08(self):
        ...
        self.database.open_database(bdb, files={"file1"})
        self.check_specification()
    ...
    def test_09(self):
        ...
        for t in (
            ...
            for v in t.values():
                if isinstance(v, list):
                    for i in v:
                        self.assertEqual(i.__class__.__name__, "str")
                        c += 1
                        o.add(i)
                else:
        ...
        for t in (d.ebm_control,):
            for v in t.values():
                self.assertEqual(v.ebm_table.__class__.__name__, "str")
                c += 1
                o.add(v)
    ...
    # Comment in _db_tkinter.py suggests this method is not needed.
...
class Database_do_database_task(unittest.TestCase):
    ...
    def setUp(self):
        class _ED(_db_tkinter.Database):
...
class Database_methods(_DBOpen):
    ...
    def create_ebm(self):
        ...
        db_tcl.tcl_tk_call(
            (
                self.database.ebm_control["file1"].ebm_table,
                "put",
                1,
                values,
            )
...
class Database_find_values(_DBOpen):
    def setUp(self):
        ...
        for key in ("d", "e", "c", "dk", "f"):
            self.database.table["file1_field1"].put(
                key.encode(), encode("values")
            )
        self.valuespec = ValuesClause()
        self.valuespec.field = "field1"
...
class Database_make_recordset(_DBOpen):
    def setUp(self):
        ...
        try:
            for e, k in enumerate(keys):
                ...
                self.cursor_put(cursor, k.encode(), self.references[k])
    ...
    def cursor_put(self, cursor, key, value):
        db_tcl.tcl_tk_call((cursor, "put", "-keylast", key, value))
    ...
    def test_13_populate_segment(self):
        ...
        try:
            while True:
                k, v = db_tcl.tcl_tk_call((cursor, "get", "-next"))[0]
                if k.decode() == "one":
    ...
    # exception is raised.
    # Why does this not raise some exception while the bsddb3 or berkeleydb
    ...
    # The '\ndb get:invalid argument' is appended extra in tcl interface via
    # tkinter compared with berkeleydb interface: caused a bit of confusion
    # when appending '$' so the regular expression means str.endswith(...).
    ...
    def test_58_database_cursor(self):
        d = self.database
        self.assertIsInstance(
            d.database_cursor("file1", "file1"), _db_tkinter.CursorPrimary
        )
        self.assertIsInstance(
...
class Database_freed_record_number(_DBOpen):
    def setUp(self):
        super().setUp()
        self.database.start_transaction()
        # An ExistenceBitmapControl instance already exists.
        # Binding another one loses the reference to the Tcl command for the
        # original without deleting and leads to an exception when closing
        ...
        self.high_record = self.database.get_high_record_number("file1")[0]
...
class RecordsetCursor(_DBOpen):
    def setUp(self):
        ...
        for i in range(380):
            db_tcl.tcl_tk_call(
                tuple(command + [encode(str(i + 1) + "Any value")])
            )
